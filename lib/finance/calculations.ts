...
export function plannedAmountForMonth(expense: RecurringExpense, targetMonth: string) {
  ...
  if (difference < 0) return expense.schedule === "english_cycle" ? expense.amount : 0;

  if (expense.schedule === "bimonthly") {
    return difference % 2 === 0 ? expense.amount : 0;
  }

  return difference % 4 === 0 ? (expense.cycleStartAmount ?? expense.amount) : expense.amount;
}

export function plannedExpensesForMonth(expenses: RecurringExpense[], targetMonth: string): PlannedExpense[] {
  return expenses
    .map((expense) => ({
      ...expense,
...
export function monthlyTotal(expenses: RecurringExpense[], targetMonth: string) {
...
export function averageMonthlyEssentialCost(expenses: RecurringExpense[]) {
  ...
  return Array.from({ length: 12 }, (_, index) => monthlyTotal(expenses, addMonths(anchor, index))).reduce(
    (total, amount) => total + amount,
    0,
  ) / 12;
}

export function balanceFromTransactions(openingBalance: number, transactions: Transaction[]) {
...
export function nextIncomeDate(today: Date) {
  const candidate = new Date(today.getFullYear(), today.getMonth(), today.getDate() <= 3 ? 3 : 18);
...
export function requestToNextIncome({
  ...
  const nextIncomeEnd = new Date(nextIncome.getFullYear(), nextIncome.getMonth(), nextIncome.getDate(), 23, 59, 59);
  ...
  const committed = bills.reduce((total, bill) => total + bill.plannedAmount, 0);
