  ...
  return (
    <main className="mx-auto min-h-screen max-w-5xl px-4 pb-28 pt-5 sm:px-7 sm:pt-8">
      ...
      <section className="mt-8">
        <div className="mb-3 flex items-center justify-between"><div><p className="text-sm font-bold text-stone-900">Bolsillos para {monthLabel(period.month)}</p><p className="text-xs text-stone-500">Separa este dinero antes de usarlo.</p></div><Landmark size={19} className="text-stone-400" /></div>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          <Pocket name="Obligaciones" amount={sumCategories(["Hogar", "Servicios", "Educacion", "Finanzas"])} color="bg-violet-500" />
          <Pocket name="Mercado" amount={sumCategories(["Mercado", "Higiene"])} color="bg-amber-400" />
          <Pocket name="Movilidad" amount={sumCategories(["Transporte"])} color="bg-sky-500" />
          <Pocket name="Gato" amount={sumCategories(["Mascota"])} color="bg-rose-400" />
          <Pocket name="Gasto libre" amount={guiltFreeAmount} color="bg-emerald-500" />
          <Pocket name="Colchon" amount={100_000} color="bg-stone-500" />
        </div>
      </section>
